...
export function PreviewWindow({ videoUrl, video, className = '' }: PreviewWindowProps) {
    ...
    return (<div className={`${className}`}>
      <div className={`relative rounded-lg overflow-hidden border ${getThemeClasses()}`}>
        {isVideoLoading ? (<div className="w-full h-32 flex items-center justify-center bg-black">
            <div className="text-white/60 text-xs">
              Loading preview...
              {video && <div className="text-xs mt-1 opacity-60">{video.title?.substring(0, 30)}...</div>}
            </div>
          </div>) : videoLoadError || !videoUrl ? (<div className="w-full h-32 flex items-center justify-center bg-black/80 text-gray-400">
            <div className="text-center">
              <p className="text-xs">{videoLoadError ? 'Load failed' : 'No preview'}</p>
              {videoLoadError && videoUrl && (<p className="text-xs opacity-60 mt-1">Check console for details</p>)}
              {!videoUrl && video && (<p className="text-xs opacity-60 mt-1">{video.title?.substring(0, 30)}...</p>)}
            </div>
          </div>) : (<>
            {/* Handle static images */}
            {isStaticImage ? (<div className="w-full h-32 bg-black flex items-center justify-center">
                <img src={videoUrl} alt="Static Preview" className="w-full h-full object-contain" style={{
                    filter: `
              ...
              </div>) : (<video ref={videoRef} className="w-full h-32 bg-black object-cover" style={{
                    filter: `
                    brightness(${videoEffects.brightness}%) 
                    contrast(${videoEffects.contrast}%) 
                    saturate(${videoEffects.saturation}%) 
            ...
            <div className="absolute inset-0 bg-black/20 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
              <div className="flex items-center space-x-2">
                ...
                {!isStaticImage && (<Button onClick={handlePlayPause} size="sm" variant="ghost" className="h-8 w-8 p-0 text-white hover:bg-white/20 bg-black/40">
                    {isPlaying ? <Pause size={14}/> : <Play size={14}/>}
                  </Button>)}
                
                {video && (<Button onClick={handleCueToProgram} size="sm" className="h-6 px-2 text-xs bg-green-600/80 hover:bg-green-600 text-white">
            ...
            {duration > 0 && !isStaticImage && (<div className="absolute bottom-0 left-0 right-0 h-1 bg-black/50">
