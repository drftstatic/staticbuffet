...
export function LiveVideoMode() {
    ...
    return (<div className={`rounded-lg p-4 transition-all duration-300 ${'bg-gray-800/50 border border-lime-500/30'}`}>
      {/* Compact Status */}
      {isLiveMode && (<div className="flex items-center justify-between mb-2">
          <div className="flex items-center space-x-1 animate-pulse">
            <div className="w-2 h-2 bg-red-500 rounded-full"></div>
            <span className="text-xs font-mono text-red-500">LIVE</span>
          </div>
          <Button onClick={addLiveVideoToQueue} size="sm" className="bg-green-600 hover:bg-green-700 text-white text-xs px-2 py-1 h-6" title="Add live video to queue">
            + Add
          </Button>
        </div>)}

      {/* Compact Camera Selection */}
      {cameras.length > 0 && !isLiveMode && (<div className="mb-2">
          <Select value={selectedCamera || (cameras.length > 0 ? (cameras[0].deviceId || 'camera-0') : 'none')} onValueChange={setSelectedCamera} disabled={isLiveMode}>
            ...
            <SelectContent>
              {cameras.length === 0 && (<SelectItem value="none">No cameras available</SelectItem>)}
              {cameras.map((camera, index) => (<SelectItem key={camera.deviceId || `camera-${index}`} value={camera.deviceId || `camera-${index}`}>
      ...
      <div className="mb-2">
        <video ref={videoRef} className="w-full h-24 bg-black rounded object-cover" style={{
            filter: isLiveMode ? `
      ...
      <div className="flex gap-1">
        {!isLiveMode ? (<Button onClick={startLiveVideo} disabled={isLoading || cameras.length === 0} size="sm" className="flex items-center gap-1 text-xs px-2 py-1 h-6">
            <Camera size={12}/>
            <span>{isLoading ? 'Starting...' : 'Start'}</span>
          </Button>) : (<>
            <Button onClick={stopLiveVideo} variant="destructive" size="sm" className="flex items-center gap-1 text-xs px-2 py-1 h-6">
              <CameraOff size={12}/>
              <span>Stop</span>
            </Button>
            
            <Button onClick={addLiveVideoToQueue} variant="secondary" size="sm" className="flex items-center gap-1 text-xs px-2 py-1 h-6">
              <Square size={12}/>
              <span>Queue</span>
            </Button>
          </>)}
      ...
      {cameras.length === 0 && (<div className={`mt-4 p-3 rounded-lg ${'bg-gray-700 text-gray-300 border border-gray-600'}`}>
          <p className="text-sm">
