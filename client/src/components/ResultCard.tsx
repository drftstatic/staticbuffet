...
export function ResultCard({ video, onSelect }: ResultCardProps) {
    ...
    return (<div className={`rounded-xl overflow-hidden shadow-lg transition-all duration-300 cursor-pointer hover:scale-[1.02] hover:shadow-2xl group relative ${isDragging ? 'opacity-50 scale-95' : ''} ${'bg-gray-800/80 backdrop-blur-sm border border-lime-500/20 hover:bg-gray-800/90'}`} onClick={() => onSelect(video)} onMouseEnter={() => setShowHoverInfo(true)} onMouseLeave={() => setShowHoverInfo(false)} draggable onDragStart={handleDragStart} onDragEnd={handleDragEnd} data-testid={`card-video-${video.identifier}`}>
      {/* Hover info overlay */}
      {showHoverInfo && (<FadeTransition className="absolute inset-0 bg-black/60 flex items-center justify-center z-10">
      ...
      <div className="relative">
        {!thumbnailLoaded && !thumbnailError ? (<ThumbnailSkeleton className="w-full h-32"/>) : (<img src={thumbnailUrl || fallbackUrl} alt={video.title} className="w-full h-32 object-cover bg-gray-200 dark:bg-gray-700" onLoad={() => setThumbnailLoaded(true)} onError={(e) => {
                if (retryCount < maxRetries) {
                    setRetryCount(prev => prev + 1);
                }
                else {
