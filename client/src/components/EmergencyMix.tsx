...
export function EmergencyMix() {
    ...
    return (<div className="flex items-center space-x-1">
      {/* Quick Emergency Mix */}
      <Button onClick={handleQuickMix} disabled={searchResults.length === 0} data-testid="button-emergency-mix" data-tour-target="emergency-mix" className={`h-8 w-8 flex items-center justify-center rounded-lg border transition-colors ${getThemeClasses()}`} title="Emergency Mix - Generate instant mix">
        <Zap size={14}/>
      </Button>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button variant="ghost" size="sm" data-testid="button-mix-settings" className="h-6 w-6 p-1 text-gray-400 hover:text-gray-200 hover:bg-white/10 rounded">
            <Settings size={12}/>
          </Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input id="duration" type="number" value={options.duration} onChange={(e) => setOptions(prev => ({ ...prev, duration: parseInt(e.target.value) || 150 }))} className="mt-1" min="30" max="600" data-testid="input-mix-duration"/>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="segmentMin" type="number" value={options.segmentLength[0]} onChange={(e) => setOptions(prev => ({
            ...prev,
            segmentLength: [parseInt(e.target.value) || 2, prev.segmentLength[1]]
              ...
              <div>
                ...
                <Input id="segmentMax" type="number" value={options.segmentLength[1]} onChange={(e) => setOptions(prev => ({
            ...prev,
            ...
            <div>
              ...
              <Input id="maxClips" type="number" value={options.maxClips} onChange={(e) => setOptions(prev => ({ ...prev, maxClips: parseInt(e.target.value) || 10 }))} className="mt-1" min="3" max="50" data-testid="input-max-clips"/>
            </div>
            ...
            <div>
              ...
              <Input id="crossfade" type="number" step="0.1" value={options.crossfadeDuration} onChange={(e) => setOptions(prev => ({ ...prev, crossfadeDuration: parseFloat(e.target.value) || 0.5 }))} className="mt-1" min="0" max="2" data-testid="input-crossfade"/>
            </div>

            <div className="flex justify-between pt-4">
              <Button variant="outline" onClick={() => setIsOpen(false)} data-testid="button-cancel-mix">
                Cancel
              </Button>
              <Button onClick={handleGenerateMix} disabled={searchResults.length === 0} data-testid="button-generate-mix" className="bg-red-600 hover:bg-red-700 dark:bg-orange-500 dark:hover:bg-orange-400 text-white dark:text-black">
                Generate Mix
              </Button>
            </div>
          </div>
