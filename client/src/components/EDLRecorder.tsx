    ...
    return (<div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          {isRecording ? (<Badge className={`${theme.recording} flex items-center space-x-1`}>
              <CircleDot size={12}/>
              <span>REC</span>
              <Timer size={12}/>
              <span>{formatDuration(recordingDuration)}</span>
            </Badge>) : (<Badge variant="outline" className="flex items-center space-x-1">
              <Square size={12}/>
              <span>Standby</span>
            </Badge>)}
          
          {eventCount > 0 && (<Badge className={theme.badge}>
              {eventCount} events
            </Badge>)}
        </div>
        ...
        <div className="flex items-center space-x-2">
          {!isRecording ? (<Dialog>
              <DialogTrigger asChild>
                <Button size="sm" className={theme.button} data-testid="button-start-edl-recording">
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input value={sessionName} onChange={(e) => setSessionName(e.target.value)} placeholder="e.g., Club Night, Wedding Set, Practice Session" data-testid="input-session-name"/>
                  </div>
                  <div>
                    <Label className="text-sm font-medium">Venue</Label>
                    <Input value={venue} onChange={(e) => setVenue(e.target.value)} placeholder="e.g., The Underground, Studio A" data-testid="input-venue"/>
                  </div>
                  <div>
                    <Label className="text-sm font-medium">Description</Label>
                    <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Optional notes about this session..." rows={3} data-testid="textarea-description"/>
                  </div>
                  <Button onClick={startRecording} className={`w-full ${theme.recording}`} data-testid="button-confirm-start-recording">
                    <CircleDot size={16} className="mr-2"/>
                    Start Recording
                  </Button>
                </div>
              </DialogContent>
            </Dialog>) : (<Button onClick={stopRecording} size="sm" variant="outline" className="border-red-500 text-red-500 hover:bg-red-500 hover:text-white" data-testid="button-stop-edl-recording">
              <Square size={14} className="mr-1"/>
              Stop
            </Button>)}

          {currentSession && (<div className="flex space-x-1">
              <Button onClick={exportEDL} size="sm" variant="outline" title="Export as readable text" data-testid="button-export-edl-text">
                <FileText size={14}/>
              </Button>
              <Button onClick={exportEDLJSON} size="sm" variant="outline" title="Export as JSON data" data-testid="button-export-edl-json">
                <Download size={14}/>
              </Button>
            </div>)}
        </div>
      ...
      {isRecording && events.length > 0 && (<Card className="border-2 border-dashed">
          ...
          <CardContent>
            <ScrollArea className="h-32">
              <div className="space-y-1">
                {events.slice(-5).map((event, index) => (<div key={event.id} className="text-xs opacity-75 flex items-center space-x-2">
      ...
      {/* Session Summary */}
      {currentSession && !isRecording && (<Card>
          <CardHeader>
