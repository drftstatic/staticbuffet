...
export function LuckyDip({ onDipResults }: LuckyDipProps) {
    ...
    return (<Button onClick={handleLuckyDip} disabled={isDipping} className={`h-8 w-8 flex items-center justify-center rounded-lg border transition-colors text-sm ${getThemeClasses()}`} data-testid="button-lucky-dip" title="Lucky Dip - Random vintage clips">
      {isDipping ? (<Loader2 size={14} className="animate-spin"/>) : (<Dices size={14}/>)}
    </Button>);
}
