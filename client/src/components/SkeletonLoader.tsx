...
export function SkeletonLoader({ variant = 'card', className = '', count = 1 }: SkeletonLoaderProps) {
  ...
  const renderSkeleton = () => {
    switch (variant) {
      case 'card':
        return (
          <div className={`rounded-lg border border-gray-300/50 overflow-hidden ${className}`}>
            {/* Thumbnail skeleton */}
      ...
      case 'thumbnail':
        return (
          <div className={`aspect-video rounded ${themeClass} ${className}`} />
        );

      case 'text':
        return (
          <div className={`h-4 ${themeClass} rounded ${className}`} />
        );

      case 'video':
        return (
          <div className={`w-full h-full rounded-lg ${themeClass} ${className}`}>
...
export function QueueItemSkeleton({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`flex items-center space-x-3 p-3 rounded-lg border border-gray-300/50 ${className}`}>
      {/* Thumbnail */}
